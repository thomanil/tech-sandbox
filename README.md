...
| [Node.js](https://nodejs.org/) 22 + npm (e.g. via [nvm](https://github.com/nvm-sh/nvm)) | Runs the Vite dev server and the web build (`npm`/`tsc`/`eslint`); matches the `node:22` build stage in the Dockerfile | web client dev **and** `error_check.sh`/`autofix_lint_formatting.sh` |
